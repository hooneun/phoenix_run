defmodule PhoenixRunWeb.UserAuthTest do
  use PhoenixRunWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias PhoenixRun.Accounts
  alias PhoenixRun.Accounts.Scope
  alias PhoenixRunWeb.UserAuth

  import PhoenixRun.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, PhoenixRunWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      PhoenixRunWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        PhoenixRunWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
